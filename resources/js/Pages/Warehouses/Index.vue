...
const openCreateModal = () => {
    ...
    form.reset();
    form.clearErrors();
    form.code = 'GD-' + Math.floor(100 + Math.random() * 900);
...
const openEditModal = (warehouse: Warehouse) => {
    ...
    form.pic_user_id = warehouse.pic_user_id || '';
...
const submitForm = () => {
    const submitData = {
        ...form.data(),
        pic_user_id: form.pic_user_id === '' ? null : Number(form.pic_user_id)
    };

    if (modalMode.value === 'create') {
        router.post(route('warehouses.store'), submitData, {
            onSuccess: () => closeModal(),
            onError: (errors) => form.setError(errors)
        });
    } else if (modalMode.value === 'edit' && selectedWarehouseId.value) {
        router.put(route('warehouses.update', selectedWarehouseId.value), submitData, {
            onSuccess: () => closeModal(),
            onError: (errors) => form.setError(errors)
        });
    }
...
const deleteWarehouse = async (id: number) => {
    if (await showConfirm('Apakah Anda yakin ingin menghapus gudang ini?')) {
...
<template>
    <Head title="Gudang/Lokasi" />
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                <div>
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        Kelola gudang penyimpanan barang dan penanggung jawab (PIC) masing-masing lokasi.
                    </p>
                </div>
        ...
        <div class="bg-surface-warm dark:bg-surface-warm p-4 rounded-2xl border border-border-warm dark:border-border-warm shadow-sm mb-6 flex flex-col md:flex-row md:items-center gap-4">
            <div class="flex-1">
                <input 
                    type="text" 
                    v-model="searchQuery" 
                    placeholder="Cari kode, nama gudang, atau alamat..." 
        ...
        <div class="bg-surface-warm dark:bg-surface-warm rounded-2xl border border-border-warm dark:border-border-warm shadow-sm overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 dark:bg-surface-warm/50 text-slate-400 dark:text-slate-500 text-xs font-semibold uppercase">
                            ...
                            <th class="p-4">PIC / Penanggung Jawab</th>
                    ...
                    <tbody class="divide-y divide-slate-150 dark:divide-slate-800 text-sm">
                        <tr v-for="(warehouse, idx) in warehouses.data" :key="warehouse.id" class="text-slate-700 dark:text-slate-300 hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                            <td class="p-4 text-slate-400">
                                {{ (warehouses.current_page - 1) * 10 + idx + 1 }}
                            </td>
                            ...
                            <td class="p-4">
                                <div class="font-medium" v-if="warehouse.pic">{{ warehouse.pic.name }}</div>
                                <div class="text-xs text-slate-450 mt-0.5" v-if="warehouse.pic">{{ warehouse.pic.email }}</div>
                                <span class="text-slate-400" v-else>Belum ditentukan</span>
                            </td>
                            <td class="p-4">
                                <span 
                                    :class="[
            ...
            <!-- Inline Pagination -->
            <div v-if="warehouses.last_page > 1" class="p-4 border-t border-border-warm dark:border-border-warm flex items-center justify-between">
                <span class="text-xs text-slate-500 dark:text-slate-400">
                    Menampilkan {{ warehouses.data.length }} dari {{ warehouses.total }} data
                </span>
            ...
            <div class="bg-surface-warm dark:bg-surface-warm border border-border-warm dark:border-border-warm rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden relative z-10">
                <div class="p-6 border-b border-border-warm dark:border-border-warm flex items-center justify-between">
                    <h3 class="font-bold text-lg text-slate-800 dark:text-slate-200">
                        {{ modalMode === 'create' ? 'Tambah Gudang' : 'Edit Gudang' }}
                    </h3>
                ...
                <form @submit.prevent="submitForm" class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="space-y-1">
                            <label for="modal-pic" class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block">
                            ...
                            <span v-if="form.errors.pic_user_id" class="text-xs text-rose-500 mt-1 block">
                    ...
                    <div class="space-y-1">
                        ...
                        <input 
                            ...
                            v-model="form.name" 
                            required
                            placeholder="Contoh: Gudang Barat, Gudang Transit"
                            class="w-full rounded-xl border-border-warm dark:border-border-warm bg-surface-warm dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200/50"
                        />
